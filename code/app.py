...
def inbound_handler(event, context):
    ...
    to_drop = False
    if event['envelope']['mailFrom']['address'].find('@'+DOMAIN)==-1:
        to_drop = True
    ...
    if to_drop:
        result['actions'][0]['action']['type'] = 'DROP'
    print(result)
    ...
    try:
        ...
        item = {
            ...
            'subject': { 'S': subject },
            'recipients': { 'S': json.dumps(event['envelope']['recipients']) },
            'response': { 'S': json.dumps(result) },
            'datetimeMessage': { 'S': datetime.utcnow().isoformat() }
        }
...
def outbound_handler(event, context):
    ...
    for recipient in event['envelope']['recipients']:
        if recipient['address'].find('@'+DOMAIN)==-1:
            drops['recipients'].append(recipient['address'])
    ...
    if len(drops['recipients'])>0:
        result['actions'].append(drops)
    print(result)
    try:
        dynamodb = boto3.client('dynamodb')
        item = {
